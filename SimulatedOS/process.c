...
void init_idle_process ()
{
  ...
  allocate_memory (idlePid, 1);
  if (Debug) dump_PCB (idlePid);

  // load instructions and data into memory
  load_instruction_idle (idlePid, 0, OPifgo, 0);
  load_instruction_idle (idlePid, 1, OPifgo, 0);
  load_data_idle (idlePid, 0, 1.0);
...
char *fname;
{
  ...
  fscanf (fprog, "%d %d %d\n", &msize, &numinstr, &numdata);
   ...
   pid = get_PCB ();
    ...
    PCB[pid]->pages_per_process= find_pages(msize, pageSize);
    
    printf("-------------------- PROCESS %d SUBMITTED..... IT NEEDS %d PAGES ------------ \n",pid,PCB[pid]->pages_per_process);
    
    if(PCB[pid]->pages_per_process>rem_pages)
    {
        printf("No sufficient frames to allocate... submission failed \n");
        return;
    }
    else
    {
        rem_pages = num_pages-PCB[pid]->pages_per_process;
    }
 
  process_counter++;
  allocate_swap_memory (pid, msize, numinstr,PCB[pid]->pages_per_process);
  allocate_spool (pid);
  ...
  { fscanf (fprog, "%d %d\n", &opcode, &operand);
    ...
    ret = load_instruction_swap (pid, i, opcode, operand);
    if (ret == mError) PCB[pid]->exeStatus = eError;
  }
  for (i=0; i<numdata; i++)
  { fscanf (fprog, "%f\n", &data);
    ret = load_data_swap (pid, i, data);
    if (Debug) printf ("Process %d load data: %d, %.2f\n", pid, i, data);
    if (ret == mError) PCB[pid]->exeStatus = eError;
  }
    allocate_memory (pid,PCB[pid]->pages_per_process);
    load_RAM(pid);
...
void load_RAM(int pid)
{
int c=0,i,firsttime=1, ret, rem_inst=0, rem_data=0;

printf("----------------------------- LOADING ONE FRAME INTO MAIN MEMORY OF PROCESS %d ----------------------\n", pid);
if(PCB[pid]->counter<PCB[pid]->numInstr)
 {firsttime=0;
       for (i=PCB[pid]->counter; i<PCB[pid]->numInstr; i++)
         {
             ret = load_instruction(pid,i, PCB[pid]->pageTableptr_swap);
             c++;
             PCB[pid]->counter++;
             if (ret == mError) PCB[pid]->exeStatus = eError;
             if(c==pageSize || c==PCB[pid]->numInstr)
              break;
        }
 }
if(c==pageSize)
...
if(c==PCB[pid]->numInstr ||  c<pageSize)
{
if(firsttime==1)
  c=0;

for(i=PCB[pid]->counter; i<=(PCB[pid]->numData+PCB[pid]->numInstr); i++)
  {
	 ret = load_data(pid,i, PCB[pid]->pageTableptr_swap);
         c++;
	 PCB[pid]->counter++;
	 if (ret == mError) PCB[pid]->exeStatus = eError;
	 if(c==pageSize)
